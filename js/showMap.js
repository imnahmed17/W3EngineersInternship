...
function getRandomValue(inputValue, range) {
    const lowerBound = inputValue - (range / 2);
    const upperBound = inputValue + (range / 2);

    let randomValue = inputValue;
...
showMapBtn.addEventListener("click", function () {
    if (resultDiv.classList.contains('hidden')) {
        ...
        .then((data) => {
            ...
            map = L.map('map').setView([singleCity.latitude, singleCity.longitude], 17);
            ...
            properties.filter(el => el.price <= max && el.price >= min).forEach((property) => {
                var customIcon = L.divIcon({
                    ...
                    html: `<div class="custom-marker w-14 py-1 bg-white text-black text-center font-semibold rounded-full">$${property.price}</div>`
                });
